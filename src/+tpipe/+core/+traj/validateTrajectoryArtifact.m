...
local_require_fields(a, required, 'tpipe:traj:MissingField');

t = a.t_s;
assert(isnumeric(t) && iscolumn(t) && ~isempty(t), ...
    'tpipe:traj:InvalidTime', 't_s must be a non-empty numeric column vector.');
assert(all(isfinite(t)) && all(diff(t) >= 0), ...
    'tpipe:traj:InvalidTime', 't_s must be finite and monotonic nondecreasing.');

n = numel(t);
local_require_matrix(a.r_eci_km, n, 3, 'r_eci_km', 'tpipe:traj:InvalidPosition');
local_require_matrix(a.v_eci_kmps, n, 3, 'v_eci_kmps', 'tpipe:traj:InvalidVelocity');

assert(islogical(a.valid_mask) && iscolumn(a.valid_mask) && numel(a.valid_mask) == n, ...
    'tpipe:traj:InvalidMask', 'valid_mask must be an Nt-by-1 logical vector.');
