...
local_require_fields(a, required, 'ttube:metrics:MissingField');

assert(isstruct(a.thresholds), ...
    'ttube:metrics:InvalidThresholds', 'thresholds must be a struct.');
assert(isstruct(a.case_metrics), ...
    'ttube:metrics:InvalidCaseMetrics', 'case_metrics must be a struct array.');
assert(isstruct(a.robust_metrics), ...
    'ttube:metrics:InvalidRobustMetrics', 'robust_metrics must be a struct.');
assert(isnumeric(a.pass_ratio) && isscalar(a.pass_ratio) && isfinite(a.pass_ratio) && ...
    a.pass_ratio >= 0 && a.pass_ratio <= 1, ...
    'ttube:metrics:InvalidPassRatio', 'pass_ratio must be a scalar fraction in [0, 1].');
