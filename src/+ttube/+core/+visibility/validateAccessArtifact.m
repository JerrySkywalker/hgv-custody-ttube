...
assert(isnumeric(t) && iscolumn(t) && ~isempty(t), ...
    'ttube:visibility:InvalidTime', 't_s must be a non-empty numeric column vector.');
assert(all(isfinite(t)) && all(diff(t) >= 0), ...
    'ttube:visibility:InvalidTime', 't_s must be finite and monotonic nondecreasing.');

nt = numel(t);
assert(islogical(a.access_mask) && ismatrix(a.access_mask) && size(a.access_mask,1) == nt, ...
    'ttube:visibility:InvalidAccessMask', 'access_mask must be Nt-by-Ns logical.');

ns = size(a.access_mask, 2);
assert(ns >= 1, 'ttube:visibility:InvalidAccessMask', 'access_mask must have at least one satellite column.');
assert(isnumeric(a.num_visible) && iscolumn(a.num_visible) && numel(a.num_visible) == nt, ...
    'ttube:visibility:InvalidVisibleCount', 'num_visible must be Nt-by-1 numeric.');
assert(islogical(a.dual_coverage_mask) && iscolumn(a.dual_coverage_mask) && ...
    numel(a.dual_coverage_mask) == nt, ...
    'ttube:visibility:InvalidDualCoverage', 'dual_coverage_mask must be Nt-by-1 logical.');
