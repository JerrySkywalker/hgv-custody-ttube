...
local_require_fields(a, required, 'ttube:orbit:MissingField');

t = a.t_s;
assert(isnumeric(t) && iscolumn(t) && ~isempty(t), ...
    'ttube:orbit:InvalidTime', 't_s must be a non-empty numeric column vector.');
assert(all(isfinite(t)) && all(diff(t) >= 0), ...
    'ttube:orbit:InvalidTime', 't_s must be finite and monotonic nondecreasing.');

ns = numel(a.sat_id);
nt = numel(t);
assert(ns >= 1, 'ttube:orbit:InvalidSatId', 'sat_id must contain at least one satellite.');
assert(numel(a.plane_index) == ns && numel(a.sat_index_in_plane) == ns, ...
    'ttube:orbit:InvalidIndex', 'plane and satellite indices must match sat_id length.');

local_require_state_array(a.r_eci_km, nt, ns, 'r_eci_km');
local_require_state_array(a.v_eci_kmps, nt, ns, 'v_eci_kmps');
assert(isstruct(a.walker), 'ttube:orbit:InvalidWalker', 'walker must be a struct.');
...
assert(isnumeric(x) && ndims(x) == 3 && size(x,1) == nt && ...
    ...
    'ttube:orbit:InvalidStateArray', '%s must be Nt-by-Ns-by-3.', label);
assert(all(isfinite(x), 'all'), ...
    'ttube:orbit:InvalidStateArray', '%s must contain finite values.', label);
end
