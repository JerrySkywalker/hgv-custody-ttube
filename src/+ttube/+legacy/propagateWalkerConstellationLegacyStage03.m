function artifact = propagateWalkerConstellationLegacyStage03(walker, t_s, cfg)
%PROPAGATEWALKERCONSTELLATIONLEGACYSTAGE03 Legacy comparison adapter.
...
t_s = satbank.t_s(:);
r_eci_km = permute(satbank.r_eci_km, [1 3 2]);
[~, ns, ~] = size(r_eci_km);
...
artifact.constellation_id = sprintf('walker_h%.0f_i%.0f_P%d_T%d_F%d', ...
    satbank.walker.h_km, satbank.walker.i_deg, satbank.walker.P, satbank.walker.T, satbank.walker.F);
...
artifact.producer = 'ttube.legacy.propagateWalkerConstellationLegacyStage03';
